...
void lcd_ss1306_i2c_dev_conf(i2c_master_bus_handle_t bus_handle) {

    i2c_device_config_t dev_cfg = {
        .dev_addr_length = I2C_ADDR_BIT_LEN_7,
        .device_address = LCD_SSD1306_ADDRESS,
        .scl_speed_hz = I2C_MASTER_FREQ_HZ,
    };

    ESP_ERROR_CHECK(i2c_master_bus_add_device(bus_handle, &dev_cfg, &oled_dev_handle));
}


esp_err_t lcd_ssd1306_is_connect(i2c_master_bus_handle_t bus_handle) {

    return i2c_master_probe(bus_handle, LCD_SSD1306_ADDRESS, 100);
}
...
void i2c_init(SSD1306_t * dev, int width, int height) {
	...
	esp_err_t res;
	res = i2c_master_transmit(oled_dev_handle, out_buf, out_index, I2C_TICKS_TO_WAIT);
	if (res == ESP_OK) {
...
void i2c_display_image(SSD1306_t * dev, int page, int seg, uint8_t * images, int width) {
	...
	res = i2c_master_transmit(oled_dev_handle, out_buf, width + 1, I2C_TICKS_TO_WAIT);
	if (res != ESP_OK)
		ESP_LOGE(TAG, "Could not write to device");
	free(out_buf);
}
...
void i2c_contrast(SSD1306_t * dev, int contrast) {
	...
	esp_err_t res = i2c_master_transmit(oled_dev_handle, out_buf, 3, I2C_TICKS_TO_WAIT);
	if (res != ESP_OK)
		ESP_LOGE(TAG, "Could not write to device");
}
...
void i2c_hardware_scroll(SSD1306_t * dev, ssd1306_scroll_type_t scroll) {
	...
	esp_err_t res = i2c_master_transmit(oled_dev_handle, out_buf, out_index, I2C_TICKS_TO_WAIT);
	if (res != ESP_OK)
		ESP_LOGE(TAG, "Could not write to device");
}
